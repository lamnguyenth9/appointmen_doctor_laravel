...
class DocsController extends Controller
{
    ...
    {
        $doctor=Auth::user();
        $appiontments=Apointment::where('doc_id',$doctor->id)->where('status','up coming')->get();
        $reviews= Reviews::where('doc_id',$doctor->id)->where('status', 'active')->get();
        return view('dashboard')->with(['doctor'=>$doctor,'apointments'=>$appiontments,'reviews'=>$reviews]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $appointment = Apointment::where('id', $request->get('appointment_id'))->first();
